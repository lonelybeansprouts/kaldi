    ...
    def train(self, model, optimizer, scheduler, data_loader, device, writer,
        ...
        with model_context():
            for batch_idx, batch in enumerate(data_loader):
                ...
                if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                    num_utts, batch_data = model.module.unpack_train_data(batch, device)
                else:
                    num_utts, batch_data = model.unpack_train_data(batch, device)
                if num_utts == 0:
                    continue
                context = None
                ...
                with context():
                    ...
                    with torch.cuda.amp.autocast(scaler is not None):
                        loss, loss_details = model(*batch_data)
                ...
                if batch_idx % log_interval == 0:
                    ...
                    for item in loss_details.items():
                        log_str += '{} {:.6f} '.format(item[0], item[1].item())
