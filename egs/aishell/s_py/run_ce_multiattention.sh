...
affix=transformer_pdf_ce_multiattention
dir=exp/py_$affix
mkdir -p $dir

train_config=conf/pyconf/train_unified_transformer_ce_multiattention.yaml
...
if [ $stage -le 6 ]; then
    ...
    mkdir -p $decode_dir
    python3 -u pynet/bin/dump_logp.py --gpu 0 \
